...
awsMock.mock('S3', 'upload', function(params, cb) {
  ...
  if(params.ACL !== 'public-read')
    return cb(new Error('ACL must be public-read'));

  if(params.Bucket !== 'fake-bucket')
    return cb(new Error('bucket must be fake-bucket'));

  if(!params.Key)
    return cb(new Error('key must be set'));

  if(!params.Body)
    return cb(new Error('body must be set'));
...
awsMock.mock('S3', 'deleteObject', function(params, cb) {
  if(!params.Key)
    return cb(new Error('must have key set'));
